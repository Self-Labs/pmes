...
async function salvarAgora() {
  ...
  try {
    ...
    if (auditEl)
      auditEl.textContent = `Última edição: ${DB.config.ultima_alteracao} por ${DB.config.alterado_por}`;
  ...
  equipes: [
    {
      ...
      nome: "Equipe A",
      offset: 1,
      militares: [null, null, null, null, null, null, null],
    },
    {
      id: "B",
      nome: "Equipe B",
...
function adicionarMilitar() {
  ...
  if (posto === "OUTRO") {
    posto = document
      .getElementById("novoPostoManual")
      .value.trim()
      .toUpperCase();
  }
...
function salvarEdicaoMilitar(id) {
  ...
  const novoPosto = document
    .getElementById("editPosto")
    .value.trim()
    .toUpperCase();
...
function renderMilitares() {
  ...
  document.getElementById("listaMilitares").innerHTML = militaresOrdenados
    .map(
      (m) => `
    <tr>
...
function renderColunas() {
  ...
  container.innerHTML = DB.colunas
    .map(
      (col, i) => `
...
function removerColuna() {
  if (DB.colunas.length <= 1) {
    alert("Mínimo 1 coluna!");
    return;
  }
  coletarColunas();
  DB.colunas.pop();
...
function removerEquipe(id) {
  if (DB.equipes.length <= 1) {
    alert("⚠️ Mínimo de 1 equipe!");
    return;
  }
...
function renderEquipes() {
          ...
          <select class="form-select equipe-select" data-equipe="${eq.id}" data-col="${i}" style="font-size: 12px;">
            ...
            ${DB.militares
              .map((m) => {
                ...
                return jaUsado
                  ? ""
                  : `<option value="${m.id}" ${selId == m.id ? "selected" : ""}>${escapeHTML(m.posto)} ${escapeHTML(m.nome)}</option>`;
              })
              .join("")}
          </select>`;
          })
  ...
  document.querySelectorAll(".equipe-select").forEach((sel) => {
    sel.addEventListener("change", (e) => {
      ...
      if (equipe) {
        equipe.militares[col] = e.target.value
...
function renderObservacoes() {
  document.getElementById("observacoesContainer").innerHTML = DB.observacoes
    .map(
      (obs, i) => `
...
function carregarConfig() {
  ...
  if (DB.config.horarios) {
    const splitTime = (str) =>
      (str || "").includes(" às ") ? str.split(" às ") : [str, ""];
  ...
  document.getElementById("footerEndereco1").value =
    DB.config.footerEndereco1 || "";
  document.getElementById("footerEndereco2").value =
    DB.config.footerEndereco2 || "";
}
...
function renderOffsetsConfig() {
  ...
  if (DB.equipes.length === 0) {
    container.innerHTML =
      '<span style="font-size:12px; color:#999;">Nenhuma equipe cadastrada.</span>';
    return;
  }

  container.innerHTML = DB.equipes
    .map(
      (eq) => `
...
async function carregarDados() {
  ...
  if (DB.config && DB.config.ultima_alteracao) {
    const auditEl = document.getElementById("auditTrail");
    if (auditEl)
      auditEl.textContent = `Última edição: ${DB.config.ultima_alteracao} por ${DB.config.alterado_por || "Desconhecido"}`;
  }
...
function renderizarDocumento() {
  ...
  const brasaoEsqHTML = c.brasaoEsq
    ? `<img src="${c.brasaoEsq}" style="height: 80px; width: auto; object-fit: contain;">`
    : '<div style="width: 80px; height: 80px; border: 1px solid #ccc; display: flex; align-items: center; justify-content: center; font-size: 11px; color: #999;">Brasão</div>';
  const brasaoDirHTML = c.brasaoDir
    ? `<img src="${c.brasaoDir}" style="height: 80px; width: auto; object-fit: contain;">`
    : '<div style="width: 80px; height: 80px; border: 1px solid #ccc; display: flex; align-items: center; justify-content: center; font-size: 11px; color: #999;">Brasão</div>';
  ...
  const formatMilitar = (id) => {
    ...
    return m
      ? `<div style="line-height: 1.2;"><span style="font-weight: bold;">${escapeHTML(m.posto)} ${escapeHTML(m.nome)}</span><br>RG ${escapeHTML(m.rg)}</div>`
      : "XXX";
  };
    ...
    <thead><tr class="bg-pm-gold">
      ...
      ${(() => {
        ...
        return headers
          .map(
            (h) =>
              `<th colspan="${h.span}" style="height: 32px; font-weight: bold; text-align: center; vertical-align: middle; text-transform: uppercase; border: 1px solid black;">${escapeHTML(h.text)}</th>`,
          )
          .join("");
      })()}
    </tr></thead>
  ...
  // Observações
  document.getElementById("observacoesTexto").innerHTML = DB.observacoes
    .filter((o) => o.trim())
    .map(
      (o) =>
        `<p style="margin-bottom: 4px; text-align: justify; line-height: 1.3;"><strong>➢</strong> ${escapeHTML(o)}</p>`,
    )
    .join("");

  // Assinatura
  document.getElementById("assinaturaTexto").textContent =
    `${c.assinaturaNome || "NOME"} – ${c.assinaturaPosto || "POSTO/GRAD"}`;
  document.getElementById("assinaturaInfoTexto").textContent =
    c.assinaturaInfo || "";
  document.getElementById("footerLemaTexto").textContent = c.footerLema || "";
  document.getElementById("footerEnderecoTexto1").textContent =
    c.footerEndereco1 || "";
  document.getElementById("footerEnderecoTexto2").textContent =
    c.footerEndereco2 || "";

  // Clone Fixo (Double-Footer Strategy)
  if (document.getElementById("footerLemaTextoFixed")) {
    document.getElementById("footerLemaTextoFixed").textContent =
      c.footerLema || "";
    document.getElementById("footerEnderecoTexto1Fixed").textContent =
      c.footerEndereco1 || "";
    document.getElementById("footerEnderecoTexto2Fixed").textContent =
      c.footerEndereco2 || "";
  }
}
...
function renderizarCalendario() {
  ...
  for (let d = diaInicio; d <= ultimoDia1; d++) {
    dias.push({
      dia: d,
      mes: mesInicio,
      diaSemana: new Date(ano, mesInicio, d).getDay(),
    });
  }
  for (let d = 1; d <= diaFim; d++) {
    dias.push({
      dia: d,
      mes: mesFim,
      diaSemana: new Date(anoFim, mesFim, d).getDay(),
    });
  }
...
document
  .querySelectorAll(
    "#tab-config input, #tab-config select, #tab-config textarea",
  )
  .forEach((el) => {
...
observer.observe(document.getElementById("colunasContainer"), {
  childList: true,
  subtree: true,
});
observer.observe(document.getElementById("equipesContainer"), {
  childList: true,
  subtree: true,
});
observer.observe(document.getElementById("observacoesContainer"), {
  childList: true,
  subtree: true,
});
