  ...
  try {
    ...
    if (user.role === "admin") {
      ...
      if (savedUnitId && unidades.some((u) => u.id === savedUnitId)) {
        ...
        currentUnidadeId =
          user.unidade_id || (unidades.length > 0 ? unidades[0].id : null);
      }
...
async function salvarAgora() {
  ...
  try {
    ...
    document.getElementById("auditTrail").textContent =
      `Última edição: ${agora} por ${user.nome}`;
  } catch (err) {
...
function carregarDadosUI() {
  ...
  document.getElementById("chkAudiencias").checked =
    c.mostrar_audiencias || false;
  ...
  // Visibilidade seções
  document
    .getElementById("secaoIseo")
    .classList.toggle("hidden", !c.mostrar_iseo);
  document
    .getElementById("secaoAudiencias")
    .classList.toggle("hidden", !c.mostrar_audiencias);
  document
    .getElementById("secaoTotais")
    .classList.toggle("hidden", !c.mostrar_totais);
  ...
  document.getElementById("outrasDeterminacoes").value =
    c.outras_determinacoes || "";
...
function coletarTudo() {
  ...
  DB.config = {
    ...
    total_atestados:
      parseInt(document.getElementById("totalAtestados").value) || 0,
    total_operacoes:
      parseInt(document.getElementById("totalOperacoes").value) || 0,
...
function carregarBrasao(lado) {
  const input = document.getElementById(
    lado === "esq" ? "brasaoEsqFile" : "brasaoDirFile",
  );
...
function renderEfetivo() {
  ...
  DB.efetivo.forEach((e) => {
    ...
    const placeholderMod =
      e.tipo === "ISEO" ? "Policiamento..." : "RO/RI/NPC...";
    tr.innerHTML = `
...
function renderizarDocumento() {
  ...
  const data = DB.data_servico
    ? new Date(DB.data_servico + "T12:00:00")
    : new Date();
  const dias = [
    "DOMINGO",
    "SEGUNDA-FEIRA",
  ...
  const headerStyle =
    "background:#e5e7eb;text-align:center;font-weight:bold;padding:4px;border:1px solid #000;font-size:12px;font-family:inherit;";
  // Estilo padrão para headers de blocos de texto (borda inferior)
  const textBlockHeaderStyle =
    "background:#e5e7eb;text-align:center;font-weight:bold;padding:4px;border-bottom:1px solid #000;font-size:12px;font-family:inherit;";
  ...
  document.getElementById("viewSubtitulo").innerHTML =
    `Serviço do dia ${dataFormatada} - <b>${diaSemana}</b>`;
  ...
  linhasExpandidas.forEach((linha, i) => {
    ...
    html += "<tr>";
    if (spanModalidade[i] > 0)
      html += `<td rowspan="${spanModalidade[i]}" class="cell-center">${escapeHTML(linha.modalidade)}</td>`;
    if (spanSetor[i] > 0)
      html += `<td rowspan="${spanSetor[i]}" class="cell-center">${escapeHTML(linha.setor)}</td>`;
    if (spanHorario[i] > 0)
      html += `<td rowspan="${spanHorario[i]}" class="cell-center">${escapeHTML(linha.horario)}</td>`;
    if (linha.milIndex === 0)
      html += `<td rowspan="${linha.milTotal}" class="cell-center">${escapeHTML(linha.viatura)}</td>`;
  ...
  if (linhasExpandidas.length === 0) {
    html +=
      '<tr><td colspan="6" style="text-align:center;color:#999;">Nenhum efetivo cadastrado</td></tr>';
  }
  ...
  if (c.mostrar_iseo) {
    ...
    linhasIseo.forEach((linha, i) => {
      ...
      iseoHtml += "<tr>";
      if (spanModalidade[i] > 0)
        iseoHtml += `<td rowspan="${spanModalidade[i]}" class="cell-center">${escapeHTML(linha.modalidade)}</td>`;
      if (spanSetor[i] > 0)
        iseoHtml += `<td rowspan="${spanSetor[i]}" class="cell-center">${escapeHTML(linha.setor)}</td>`;
      if (spanHorario[i] > 0)
        iseoHtml += `<td rowspan="${spanHorario[i]}" class="cell-center">${escapeHTML(linha.horario)}</td>`;
      if (linha.milIndex === 0)
        iseoHtml += `<td rowspan="${linha.milTotal}" class="cell-center">${escapeHTML(linha.viatura)}</td>`;
    ...
    if (linhasIseo.length === 0) {
      iseoHtml +=
        '<tr><td colspan="6" style="text-align:center;color:#999;">Nenhum registro</td></tr>';
    }
  ...
  if (c.mostrar_audiencias && DB.audiencias.length > 0) {
    ...
    audHtml +=
      '</tbody></table><p style="font-size:10px;font-style:italic;padding:4px;">Obs.: Este não é o meio oficial de intimação.</p>';
  ...
  if (c.mostrar_rodape) {
    ...
    document.getElementById("viewRodape").innerHTML = rodapeHtml;
    // Clone Fixo (Double-Footer Strategy)
    if (document.getElementById("viewRodapeFixed")) {
      document.getElementById("viewRodapeFixed").innerHTML = rodapeHtml;
    }
  } else {
    document.getElementById("viewRodape").innerHTML = "";
    if (document.getElementById("viewRodapeFixed")) {
      document.getElementById("viewRodapeFixed").innerHTML = "";
    }
  }
}
...
function calcularRowspans(dados, campo) {
  ...
  while (i < dados.length) {
    let count = 1;
    while (
      i + count < dados.length &&
      dados[i + count][campo] === dados[i][campo]
    ) {
      count++;
    }
...
document
  .querySelectorAll(
    "#tab-config input, #tab-config textarea, #tab-config select",
  )
  .forEach((el) => {
...
document
  .querySelectorAll("#tab-observacoes input, #tab-observacoes textarea")
  .forEach((el) => {
    el.addEventListener("input", marcarAlterado);
  });
...
async function importarDaMensal() {
  ...
  if (!diaInput.value)
    return alert("Por favor, selecione uma data para importar.");
  ...
  if (
    !confirm(
      `Deseja importar o efetivo da Escala Mensal para o dia ${dia}?\nIsso adicionará novos registros ao efetivo atual.`,
    )
  )
    return;
  ...
  try {
    ...
    const resp = await fetch(
      `/api/escalas/mensal?unidade_id=${currentUnidadeId}`,
      {
...
function processarImportacaoMensal(mensal, dia) {
  ...
  if (diaIndex === -1) {
    alert(
      `O dia ${dia} não foi encontrado no período configurado da escala mensal.`,
    );
    return;
  }
  ...
  equipes.forEach((eq) => {
    ...
    if (turno) {
      const horario =
        horarios[turno] || (turno === "D" ? "06:00-18:00" : "18:00-06:00");
...
function ordenarEfetivo() {
  DB.efetivo.sort((a, b) => {
    // 1. ISEO por último
    if (a.tipo !== b.tipo)
      return (a.tipo === "ISEO" ? 1 : -1) - (b.tipo === "ISEO" ? 1 : -1);
