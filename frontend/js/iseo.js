...
function adicionarMilitar() {
  ...
  const nomeGuerra = document
    .getElementById("novoNomeGuerra")
    .value.trim()
    .toUpperCase();
  ...
  if (!posto || !nome || !rg) {
    alert("Preencha Posto, Nome e RG!");
    return;
  }
...
function moverMilitar(id, direcao) {
  ...
  [DB.militares[idx], DB.militares[novoIdx]] = [
    DB.militares[novoIdx],
    DB.militares[idx],
  ];
  renderMilitares();
  marcarAlterado();
...
function renderMilitares() {
  document.getElementById("listaMilitares").innerHTML = DB.militares
    .map((m, i) => {
      let nomeExibir = escapeHTML(m.nome);
      if (m.nomeGuerra) {
        nomeExibir = escapeHTML(m.nome).replace(
          new RegExp(`(${escapeHTML(m.nomeGuerra)})`, "i"),
          "<strong>$1</strong>",
        );
      }
      return `
...
function carregarDadosUI() {
  if (DB.config.headerLinha1) {
    ...
    document.getElementById("assinaturaNome").value =
      DB.config.assinaturaNome || "";
    document.getElementById("assinaturaPosto").value =
      DB.config.assinaturaPosto || "";
    document.getElementById("assinaturaCargo").value =
      DB.config.assinaturaCargo || "";
    document.getElementById("footerLema").value = DB.config.footerLema || "";
    document.getElementById("footerLinha1").value =
      DB.config.footerLinha1 || "";
    document.getElementById("footerLinha2").value =
      DB.config.footerLinha2 || "";
    document.getElementById("footerLinha3").value =
      DB.config.footerLinha3 || "";
  }
  ...
  if (DB.dados.dataEscala) {
    ...
    document.getElementById("horarioInicio").value =
      DB.dados.horarioInicio || "";
    document.getElementById("horarioFim").value = DB.dados.horarioFim || "";
    document.getElementById("horarioApresentacao").value =
      DB.dados.horarioApresentacao || "";
    document.getElementById("localApresentacao").value =
      DB.dados.localApresentacao || "";
  }
...
function renderizarDocumento() {
  ...
  // Título
  document.getElementById("tituloOperacao").textContent =
    d.nomeOperacao || "OPERAÇÃO";
  document.getElementById("subtituloISEO").textContent = d.rubricaISEO || "";

  // Info
  const dataFormatada = d.dataEscala
    ? d.dataEscala.split("-").reverse().join("/")
    : "";
  ...
  const horarioEscala =
    d.horarioInicio && d.horarioFim
      ? `${formatarHora(d.horarioInicio)} às ${formatarHora(d.horarioFim)}`
      : "";
  const horarioApres = d.horarioApresentacao
    ? `${formatarHora(d.horarioApresentacao)}${d.localApresentacao ? " - " + d.localApresentacao : ""}`
    : "";
  ...
  DB.militares.forEach((m, i) => {
    const postoQuadro = m.quadro
      ? `${escapeHTML(m.posto)} ${escapeHTML(m.quadro)}`
      : escapeHTML(m.posto);
  ...
  document.getElementById("assinaturaTexto").textContent =
    `${c.assinaturaNome || ""} – ${c.assinaturaPosto || ""}`;
  document.getElementById("assinaturaCargoTexto").textContent =
    c.assinaturaCargo || "";

  // Rodapé
  document.getElementById("footerLemaTexto").textContent = c.footerLema || "";
  document.getElementById("footerLinha1Texto").textContent =
    c.footerLinha1 || "";
  document.getElementById("footerLinha2Texto").textContent =
    c.footerLinha2 || "";
  document.getElementById("footerLinha3Texto").textContent =
    c.footerLinha3 || "";
  ...
  if (document.getElementById("footerLemaTextoFixed")) {
    document.getElementById("footerLemaTextoFixed").textContent =
      c.footerLema || "";
    document.getElementById("footerLinha1TextoFixed").textContent =
      c.footerLinha1 || "";
    document.getElementById("footerLinha2TextoFixed").textContent =
      c.footerLinha2 || "";
    document.getElementById("footerLinha3TextoFixed").textContent =
      c.footerLinha3 || "";
  }
}
...
document
  .querySelectorAll("#tab-dados input, #tab-dados select")
  .forEach((el) => {
    el.addEventListener("input", marcarAlterado);
    el.addEventListener("change", marcarAlterado);
  });
